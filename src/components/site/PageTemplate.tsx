...
function renderBlock(block: ContentBlock) {
  if (block.type === "paragraphs") {
    return (
      <section
        key={block.title}
        className="rounded-[2rem] border border-border bg-white p-8 shadow-[0_12px_40px_rgba(15,29,74,0.05)] md:p-10"
      >
      ...
      <section key={block.title} className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {block.items.map((item) => (
            <div
              key={item.label}
              className="rounded-[1.75rem] border border-border bg-light p-6"
            >
      ...
      <section key={block.title} className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {block.items.map((item) => (
            <div
              key={item.title}
              className="rounded-[1.75rem] border border-border bg-white p-6 shadow-[0_12px_36px_rgba(15,29,74,0.04)]"
            >
      ...
      <section key={block.title} className="space-y-6">
        ...
        <div className="grid gap-4">
          {block.items.map((item, index) => (
            <div
              key={item.title}
              className="grid gap-5 rounded-[1.75rem] border border-border bg-white p-6 shadow-[0_12px_36px_rgba(15,29,74,0.04)] md:grid-cols-[84px_1fr]"
            >
      ...
      <section key={block.title} className="space-y-6">
        ...
        <div className="space-y-3">
          {block.items.map((item) => (
            <details
              key={item.question}
              className="group rounded-[1.6rem] border border-border bg-white p-6 shadow-[0_12px_36px_rgba(15,29,74,0.04)]"
            >
  ...
  return (
    <section
      key={block.title}
      className="rounded-[2rem] border border-border bg-light p-8 md:p-10"
    >
      <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.26em] text-primary/55">
        Pôvodný obsah
      </p>
...
export default function PageTemplate({ page }: PageTemplateProps) {
  ...
  return (
    <main className="bg-[linear-gradient(180deg,#f5f7fb_0%,#ffffff_18%,#ffffff_100%)]">
      ...
      <div className="mx-auto w-[95vw] px-6 pb-20 pt-12 md:px-10 md:pb-24 md:pt-16">
