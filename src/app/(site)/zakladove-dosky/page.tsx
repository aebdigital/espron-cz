...
const INCLUDED = [
  ...
  {
    label: "Shrnutí štěrku pod základovou deskou",
    icon: (
  ...
  {
    label: "Kari sítě a vyztužení roxory",
    icon: (
...
const INCLUDED_ORDER = [
  "Vytyčení polohy",
  "Shrnutí ornice",
  "Výkopové práce",
  "Shrnutí štěrku pod základovou deskou",
  "Osazení hromosvodu",
  "Zalití základových pásů",
  "Zaměření geodetem",
  "DT tvárnice - 2 řady",
  "Kari sítě a vyztužení roxory",
  "Vylití desky",
] as const;

const INCLUDED_BY_LABEL = new Map(INCLUDED.map((item) => [item.label, item]));

const ORDERED_INCLUDED = INCLUDED_ORDER.map((label) => {
  const item = INCLUDED_BY_LABEL.get(label);

  if (!item) {
    throw new Error(`Missing included item: ${label}`);
  }
      ...
      <section className="py-20 md:py-28">
        <div className="mx-auto w-[92%]">
          ...
          <div className="grid grid-cols-2 gap-8 md:grid-cols-5">
            {ORDERED_INCLUDED.map((item, index) => (
